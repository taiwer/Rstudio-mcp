...
class RStudioConfig(BaseModel):
    ...
    installation_path: Optional[str] = Field(
        default=None, description="Path to RStudio installation"
    )
    default_r_version: str = Field(
        default="4.3.0", description="Default R version to use"
    )
    r_home: Optional[str] = Field(
        default=None, description="R_HOME environment variable override"
    )
...
class EnvironmentConfig(BaseModel):
    ...
    auto_cleanup: bool = Field(
        default=True, description="Automatically cleanup unused environments"
    )
    max_environments: int = Field(
        default=10, description="Maximum number of environments to maintain"
    )
...
class SecurityConfig(BaseModel):
    ...
    execution_timeout: int = Field(
        default=300, description="Code execution timeout in seconds"
    )
...
class ServerConfig(BaseModel):
    ...
    auto_start_rstudio: bool = Field(
        default=False, description="Automatically start RStudio if not running"
    )
    ...
    def to_yaml(self, config_path: Union[str, Path]) -> None:
        ...
        with open(config_path, "w", encoding="utf-8") as f:
            yaml.dump(
                self.dict(), f, default_flow_style=False, allow_unicode=True, indent=2
            )
