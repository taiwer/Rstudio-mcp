...
from .security import (
    ...
    ResourceType, Permission, SecurityLevel
)
...
class RStudioMCPServer:
    ...
    def __init__(self, config: ServerConfig):
        ...
        self.access_controller = AccessController(config.security.dict() if hasattr(config, 'security') else {})
        self.code_validator = CodeSecurityValidator(config.security.dict() if hasattr(config, 'security') else {})
        ...
        self.logger.info(f"RStudio MCP Server initialized: {config.name} v{config.version}")
    ...
    def _start_monitoring(self) -> None:
        ...
        try:
            ...
            self._cache_manager.create_cache("tool_results", max_size=500, max_memory_mb=50)
            self._cache_manager.create_cache("resource_data", max_size=200, max_memory_mb=100)
            self._cache_manager.create_cache("prompt_templates", max_size=100, max_memory_mb=10)
    ...
    def _check_server_health(self) -> bool:
        ...
        try:
            ...
            return (
                ...
                self.resource_manager is not None and
                self.prompt_manager is not None and
                self._performance_monitor is not None
            )
        except Exception:
            return False
        ...
        def get_operation_metrics(operation_name: str = None) -> str:
            ...
            for name, op_stats in stats.items():
                if op_stats:
                    ...
                    result.append(f"- Error rate: {(op_stats.error_count / op_stats.total_calls * 100):.1f}%")
                    result.append("")
    ...
    def _setup_tool_handlers(self) -> None:
        ...
        @self.server.tool()
        async def execute_managed_tool(name: str, arguments: dict = None, session_id: str = None) -> str:
            ...
            try:
                ...
                if not self.access_controller.check_access(
                    ...
                    self.audit_logger.log_access_denied(
                        session_id, None, "tool", name, "execute", "Insufficient permissions"
                    )
                    return error_msg
                    ...
                    if violations:
                        ...
                        if critical_violations:
                            ...
                            self.audit_logger.log_security_violation(
                                session_id, None, "code_execution", "critical",
                        ...
                        for violation in violations:
                            self.audit_logger.log_security_violation(
                                session_id, None, "code_execution", violation.level.value,
                                violation.message,
                                {"code_snippet": code[:100], "line_number": violation.line_number}
                            )
                ...
                self.audit_logger.log_tool_execution(
                    session_id, None, name, arguments,
                    ...
                    if result.content:
                        ...
                        for item in result.content:
                            if item.get("type") == "text":
                                ...
                                content_parts.append(f"[Resource: {resource.get('uri', 'unknown')}]")
                ...
                execution_time = time.time() - start_time
                self.audit_logger.log_tool_execution(
                    session_id, None, name, arguments, "error", execution_time,
                    {"error": str(e)}
                )
    ...
    def _setup_resource_handlers(self) -> None:
        ...
        async def list_available_resources() -> str:
            ...
            for resource in resources:
                resource_info.append(f"- {resource.uri}: {resource.name} ({resource.mime_type})")
        ...
        async def read_managed_resource(uri: str) -> str:
            ...
            if result.success:
                if result.is_text():
                    return result.get_text_content() or "Resource read successfully (no text content)"
        ...
        def list_available_prompts() -> str:
            ...
            for prompt_name in prompts:
                ...
                if info:
                    ...
                    prompt_info.append(f"- {prompt_name}: {info['description']} (args: {args_desc})")
                else:
    ...
    async def run_sse(self, host: str = "localhost", port: int = 8080) -> None:
        ...
        self.logger.info(f"Starting MCP server with enhanced SSE transport on {host}:{port}")
    ...
    async def _handle_sse_mcp_request(self, message_data: Dict[str, Any], connection_id: str) -> None:
        ...
        try:
            ...
            if self.sse_transport:
                await self.sse_transport.send_to_connection(connection_id, error_response)
    ...
    async def _handle_sse_connection_error(self, error: Exception) -> None:
        ...
        if self.sse_transport and hasattr(self.sse_transport, 'connection_manager'):
            connection_count = self.sse_transport.connection_manager.get_connection_count()
    ...
    async def shutdown(self) -> None:
        ...
        if hasattr(self, '_error_recovery_manager') and self._error_recovery_manager:
            try:
                ...
                self.logger.info(f"Final error statistics: {stats['total_errors']} errors, "
